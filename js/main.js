...
function createOkbSupabaseClient(accessToken = "") {
  const options = accessToken
    ? { global: { headers: { Authorization: `Bearer ${accessToken}` }, fetch: okbFetchWithSessionRetry } }
    : {};
...
function stopOkbSessionRefresh() {
  if (okbSessionRefreshTimer) clearTimeout(okbSessionRefreshTimer);
  okbSessionRefreshTimer = null;
}

function scheduleOkbSessionRefresh(expiresAt) {
  stopOkbSessionRefresh();
  const expiry = Number(expiresAt || sessionStorage.getItem('okb_access_expires_at') || 0);
  if (!expiry) return;
  const delay = Math.max(1000, (expiry * 1000) - Date.now() - (10 * 60 * 1000));
  okbSessionRefreshTimer = setTimeout(() => refreshOkbSecureSession().catch(handleOkbSessionExpiry), delay);
}

async function refreshOkbSecureSession() {
  if (okbSessionRefreshPromise) return okbSessionRefreshPromise;
  const oldToken = sessionStorage.getItem('okb_access_token');
  if (!oldToken) throw new Error('لا توجد جلسة لتجديدها');
  okbSessionRefreshPromise = (async () => {
    const response = await fetch(OKB_LOGIN_ENDPOINT, {
      method:'POST', cache:'no-store',
      headers:{ 'Content-Type':'application/json', apikey:OKB_SUPABASE_KEY },
      body:JSON.stringify({ action:'refresh', token:oldToken })
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok || !result?.token || !result?.expires_at) throw new Error(result?.error || 'تعذر تجديد الجلسة');
    sessionStorage.setItem('okb_access_token', result.token);
    sessionStorage.setItem('okb_access_expires_at', String(result.expires_at));
    setOkbSecureSession(result.token, result.expires_at);
    return result.token;
  })().finally(() => { okbSessionRefreshPromise = null; });
  return okbSessionRefreshPromise;
}

function handleOkbSessionExpiry(error) {
  console.warn('Secure session refresh failed:', error);
  stopOkbSessionRefresh();
  ...
  if (typeof app !== 'undefined') app?.classList.add('hidden');
  if (typeof loginPage !== 'undefined') loginPage?.classList.remove('hidden');
  if (typeof showLoginMessage === 'function') showLoginMessage('انتهت الجلسة الآمنة. سجل الدخول مرة أخرى للمتابعة.');
}

async function okbFetchWithSessionRetry(input, init = {}) {
  let response = await fetch(input, init);
  if (response.status !== 401) return response;
  const message = await response.clone().text().catch(() => '');
  if (!/jwt\s*expired|token.*expired/i.test(message)) return response;
  try {
    const token = await refreshOkbSecureSession();
    const headers = new Headers(init.headers || (input instanceof Request ? input.headers : undefined));
    headers.set('Authorization', `Bearer ${token}`);
    response = await fetch(input, { ...init, headers });
  } catch (error) {
    handleOkbSessionExpiry(error);
  }
  return response;
}
...
document.addEventListener('visibilitychange', () => {
  ...
  if (!expiresAt) return;
  if ((expiresAt * 1000) - Date.now() <= 15 * 60 * 1000) {
    refreshOkbSecureSession().catch(handleOkbSessionExpiry);
  } else {
    scheduleOkbSessionRefresh(expiresAt);
  }
});
...
const historicalOrderSearchState = {
  dashboard:{timer:null,token:0,addedIds:new Set()},
  branch:{timer:null,token:0,addedIds:new Set()},
  khazna:{timer:null,token:0,addedIds:new Set()}
};
...
function getCairoDateRangeUTC(fromDate,toDate=fromDate){
  const parseDay=value=>{const match=String(value||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);return match?{year:Number(match[1]),month:Number(match[2]),day:Number(match[3])}:null;};
  const cairoMidnightUtc=value=>{
    const target=parseDay(value);if(!target)return null;
    const targetMs=Date.UTC(target.year,target.month-1,target.day,0,0,0,0);
    let instant=new Date(targetMs);
    for(let pass=0;pass<3;pass+=1){
      const parts=new Intl.DateTimeFormat('en-US',{timeZone:'Africa/Cairo',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'}).formatToParts(instant);
      const map=Object.fromEntries(parts.map(part=>[part.type,part.value]));
      const representedMs=Date.UTC(Number(map.year),Number(map.month)-1,Number(map.day),Number(map.hour),Number(map.minute),Number(map.second));
      instant=new Date(instant.getTime()-(representedMs-targetMs));
    }
    return instant;
  };
  const start=cairoMidnightUtc(fromDate),to=parseDay(toDate);if(!start||!to)return null;
  const nextDay=new Date(Date.UTC(to.year,to.month-1,to.day)+86400000);
  const nextDayText=`${nextDay.getUTCFullYear()}-${String(nextDay.getUTCMonth()+1).padStart(2,'0')}-${String(nextDay.getUTCDate()).padStart(2,'0')}`;
  const endExclusive=cairoMidnightUtc(nextDayText);if(!endExclusive)return null;
...
function openActivityLogTicket(encodedLogId){
  if(!hasRoleFeature('activity_log')) return;
  ...
  openReportOrderTab('activity_log',null,logId);
}
...
function parseOrderProductsForReports(order){
  ...
  if(!out.length && getEffectiveOrderPrice(order)>0) {
    const effectivePrice=getEffectiveOrderPrice(order);
    out.push({name:'منتج غير محدد',price:effectivePrice,qty:Number(order.quantity||1)||1,total:effectivePrice});
  }
  return out;
}
...
function ensureProductReportEnhancements(){
  ...
  if(head && !$('productReportHeaderActions')){ const old=head.querySelector('button[data-app-back]'); if(old)old.remove(); const a=document.createElement('div'); a.id='productReportHeaderActions'; a.style.cssText='display:flex;gap:8px;align-items:center;flex-wrap:wrap;justify-content:flex-start;'; a.innerHTML=`<button class="soft" type="button" onclick="printProductReports()">🖨 Print</button><button class="soft" type="button" onclick="exportProductReportExcel()">⬇ Export Excel</button><div class="app-refresh-back"><button class="soft" type="button" onclick="refreshProductReports(event)">Refresh</button><button class="app-back-btn" type="button" data-app-back onclick="goBackToPreviousPage()" aria-label="الرجوع إلى الصفحة السابقة"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 5 3 12l7 7M4 12h17"/></svg><span>Back</span></button></div>`; head.appendChild(a); }
...
function updateProductReportKPIs(){
  ...
  const totalRevenue=(productReportRows._filteredOrders||[]).filter(o=>!['Returned','Cancel'].includes(String(o.status||''))).reduce((s,o)=>s+getEffectiveOrderPrice(o),0);
...
async function showProductReportsPage(){ if(!canViewProductReports()){alert('غير مسموح لك بفتح Product Reports');return;} hideAllPages(); $('productReportsPage')?.classList.remove('hidden'); setActiveMenu('productReportsPage'); ensureProductReportEnhancements(); const range=getCurrentDashboardMonthRange(); $('productReportFrom').value=range.from; $('productReportTo').value=range.to; const loads=[]; if(ordersDataScope!=='all')loads.push(loadOrders()); loads.push(ensureChartLibrary()); await Promise.allSettled(loads); populateProductReportFilters(); renderProductReports(); }
function applyProductReportFilters(){ renderProductReports(); }
function resetProductReportFilters(){ const range=getCurrentDashboardMonthRange(); $('productReportFrom').value=range.from; $('productReportTo').value=range.to; if($('productReportBranch'))$('productReportBranch').value='all'; if($('productReportDoctor'))$('productReportDoctor').value='all'; if($('productReportSearch'))$('productReportSearch').value=''; renderProductReports(); }
...
const ROLE_PERMISSION_FEATURES = [
  ...
  { key:'operation_manager_report', label:'Operation Manager Report', group:'Main' },
  ...
  { key:'stores_report', label:'OKB Abnormal', group:'OKB Stores' },
  ...
  { key:'btn_branch_edit', label:'✏️ تعديل', group:'الفروع — إدارة' },
  { key:'btn_branch_transfer', label:'🔄 تحويل', group:'الفروع — إدارة' },
  { key:'btn_delete_upfront_proof', label:'🖼️ حذف إثبات السكرتارية', group:'الفروع — إدارة' },
  { key:'btn_branch_shipping_rank', label:'🚚 Shipping Rank', group:'الفروع — إدارة' },
  { key:'btn_branch_khazna', label:'🏦 فتح الخزنة', group:'الخزنة — تحكم' },
...
let rolePermissionsLoadSequence = 0;
let currentUserAccessSyncPromise = null;
let usersLoadSequence = 0;
...
async function loadRolePermissions({fresh=false,throwOnError=false}={}) {
  const actorId=String(currentUser?.id||'');
  const request=++rolePermissionsLoadSequence;
  const cached = {};
  if(!fresh)try {
  ...
  if(request!==rolePermissionsLoadSequence||String(currentUser?.id||'')!==actorId)return false;
  if (error) {
    rolePermissionsTableReady = false;
    console.warn('Role permissions table is not ready:', error.message);
    if(throwOnError)throw new Error('تعذر تحديث الصلاحيات: '+error.message);
    return false;
  }
  ...
  syncDynamicRoleCatalog();
  populateUserRoleSelects();
  try { localStorage.removeItem('okb_role_permissions_cache');localStorage.setItem('okb_role_permissions_cache', JSON.stringify(rolePermissionsByRole)); } catch (e) {}
  return true;
}
...
function currentPageStillAllowed() {
  const checks = [
    ...
    ['activityLogPage','activity_log'], ['productReportsPage','product_reports'], ['chatPage','chat'], ['secretaryAuditPage','secretary_audit']
  ];
...
function startRolePermissionsSync() {
  stopRolePermissionsSync();
  if (!currentUser) return;
  rolePermissionsSyncTimer = setInterval(() => {
    syncCurrentUserPermissions().catch(error=>console.warn('Access sync:',error?.message||error));
  }, 10000);
  if (supabaseClient?.channel) {
    rolePermissionsRealtimeChannel = supabaseClient
      ...
      .on('broadcast', { event:'permissions_updated' }, async () => {
        try{await syncCurrentUserPermissions({forceRender:true});}
        catch(error){console.warn('Access refresh:',error?.message||error);}
      })
      .subscribe();
  }
}

async function syncCurrentUserPermissions({forceRender=false}={}) {
  if(!currentUser)return false;
  if(currentUserAccessSyncPromise)return currentUserAccessSyncPromise;
  const actor=currentUser;
  currentUserAccessSyncPromise=(async()=>{
    const {data:profile,error}=await supabaseClient.from('user')
      .select('id,name,username,role,active,managed_branches,system_permissions').eq('id',currentUser.id).maybeSingle();
    if(currentUser!==actor)return false;
    if(error)throw new Error('تعذر تحديث بيانات الحساب: '+error.message);
    if(!profile||profile.active===false){handleOkbSessionExpiry(new Error('الحساب غير متاح أو تم إيقافه'));return false;}
    const previous=currentUser;
    const previousPermissions=JSON.stringify(getPermissionsForRole(previous.role));
    const before=JSON.stringify([previous.role,previous.managed_branches,previous.system_permissions]);
    const refreshed=await loadRolePermissions({fresh:true,throwOnError:true});
    if(!refreshed||currentUser!==previous)return false;
    const nextUser={...previous,name:profile.name,username:profile.username,role:profile.role,active:profile.active,
      managed_branches:profile.managed_branches??null,system_permissions:profile.system_permissions??{}};
    const scopeChanged=before!==JSON.stringify([nextUser.role,nextUser.managed_branches,nextUser.system_permissions]);
    const changed=scopeChanged||previousPermissions!==JSON.stringify(getPermissionsForRole(nextUser.role));
    const profileChanged=JSON.stringify(previous)!==JSON.stringify(nextUser);
    // Keep the same object on an unchanged poll: report loads use its identity
    // to reject responses from an older login or an earlier permission scope.
    ...
    if(changed||profileChanged||forceRender){
      ...
      const branchClosed=(isLivePageVisible('branchPage')||isLivePageVisible('khaznaPage'))&&(!canOpenPermissionBranch(currentBranchName)||!canAccessBranch(currentBranchName));
      const commissionClosed=isLivePageVisible('commissionPage')&&!hasRoleFeature('commission');
      if(!currentPageStillAllowed()||branchClosed||commissionClosed||(changed&&!isAdmin()&&!isLivePageVisible('usersPage'))){
        hideAllPages();await showInitialPermittedPage();
      }else{
        if(isLivePageVisible('ordersPage'))renderOrders();
        if(isLivePageVisible('branchPage'))renderBranchOrders();
      }
    }
    return true;
  })().finally(()=>{currentUserAccessSyncPromise=null;});
  return currentUserAccessSyncPromise;
}

async function refreshUsersPage(button) {
  if(!isAdmin()||usersRefreshInProgress)return;
  usersRefreshInProgress=true;
  const status=$('usersRefreshStatus');
  if(button){button.disabled=true;button.setAttribute('aria-busy','true');}
  if(status)status.textContent='جاري تحديث المستخدمين والصلاحيات...';
  try{
    const refreshed=await syncCurrentUserPermissions({forceRender:true});
    if(!refreshed||!isAdmin())throw new Error('تغيّرت صلاحية الحساب؛ أعد فتح الصفحة المسموح بها.');
    if(!await loadUsers({throwOnError:true}))throw new Error('لم يكتمل تحديث المستخدمين؛ حاول مرة أخرى.');
    populateUserRoleSelects();populatePermissionRoleSelect();applyUsersFormRoleLock();
    try{await rolePermissionsRealtimeChannel?.send({type:'broadcast',event:'permissions_updated',payload:{}});}
    catch(error){console.warn('Access notification:',error?.message||error);}
    if(status)status.textContent='تم تحديث المستخدمين والصلاحيات';
  }catch(error){if(status)status.textContent=error?.message||'تعذر التحديث';}
  finally{
    usersRefreshInProgress=false;
...
function syncDynamicRoleCatalog(){
  const builtinRoles=ROLE_PERMISSION_ROLES.filter(role=>BUILTIN_PERMISSION_ROLE_KEYS.has(role.key));
  ROLE_PERMISSION_ROLES.splice(0,ROLE_PERMISSION_ROLES.length,...builtinRoles);
...
async function saveRolePermissions() {
  ...
  const { error } = await supabaseClient.from('role_permissions').upsert(payload, { onConflict:'role' });
  if (error) {
    if (status) { status.textContent = 'تعذر الحفظ'; status.className='permission-save-error'; }
  ...
  if (status) { status.textContent = 'تم الحفظ بنجاح ✓'; status.className='permission-save-success'; }
  try {
...
function getOrderEditCount(order) {
  return Math.max(0, Number(getOrderMeta(order).edit_count || 0));
}
...
function getOrderFlagMeta(order) {
  ...
  return {
    ...
    errorOrder: meta.error_order === true
  };
}
...
function isErrorOrder(order) {
  return getOrderFlagMeta(order).errorOrder === true;
}
...
async function resetDateFilter() {
  ...
  const doctorFilterEl=document.getElementById('filterDoctor');
  if(doctorFilterEl)doctorFilterEl.value='الكل';
  resetAllPages();
...
async function checkLogin() {
  ...
  if (sessionIsValid) {
    setOkbSecureSession(token, expiresAt);
    currentUser = JSON.parse(s);
    // Renew a nearly-expired restored session before any doctors/orders requests start.
    // This avoids opening the app with a token that expires during initial data loading.
    if ((expiresAt * 1000) - Date.now() <= 15 * 60 * 1000) {
      try {
        await refreshOkbSecureSession();
      } catch (error) {
        handleOkbSessionExpiry(error);
        return;
      }
...
async function loadUsers({throwOnError=false}={}) {
  const actorId=String(currentUser?.id||'');
  const request=++usersLoadSequence;
  if(!actorId||(!hasRoleFeature('users')&&!isExecutiveAssistant()))return false;
  const data=[];
  let includeLastSeen=true;
  try{
    for(let offset=0;;offset+=1000){
      const columns='id,name,username,role,active,managed_branches,system_permissions';
      let result=await supabaseClient.from('user').select(columns+(includeLastSeen?',last_seen_at':''))
        .order('name',{ascending:true}).order('id',{ascending:true}).range(offset,offset+999);
      if(result.error&&includeLastSeen&&(/last_seen_at/i.test(String(result.error.message||''))||['42703','PGRST204'].includes(String(result.error.code||'')))){
        includeLastSeen=false;
        result=await supabaseClient.from('user').select(columns).order('name',{ascending:true}).order('id',{ascending:true}).range(offset,offset+999);
      }
      if(result.error)throw result.error;
      if(request!==usersLoadSequence||String(currentUser?.id||'')!==actorId||(!hasRoleFeature('users')&&!isExecutiveAssistant()))return false;
      data.push(...(result.data||[]));
      if(!result.data||result.data.length<1000)break;
    }
  }catch(error){
    if(throwOnError)throw new Error('مشكلة في تحميل المستخدمين: '+error.message);
    alert('مشكلة في تحميل المستخدمين: '+error.message);return false;
  }
...
async function loadDoctors() {
  ...
  if (error) {
    console.warn("Doctors load failed:", error.message);
    if (!doctorsList.length) {
      try {
        const cachedDoctors = JSON.parse(localStorage.getItem('okb_doctors_cache') || '[]');
...
async function loadPendingOrders(showMessage){
  ...
  try{
    ...
    if(request===pendingLoadSequence&&String(currentUser?.id||'')===actorId){
      pendingOrders=[];renderPendingOrders();
      if(list)list.innerHTML=`<div class="pending-empty">تعذر تحميل البيانات: ${escapeHTML(e.message||String(e))}</div>`;
    }
    return false;
  }finally{if(request===pendingLoadSequence&&exportButton)exportButton.disabled=!pendingDataReady||pendingExportInProgress;}
}
...
async function exportPendingOrders(button){
  if(!hasRoleFeature('pending')){alert('غير مسموح بتصدير بيانات Pending');return;}
  if(pendingExportInProgress)return;
  if(!pendingDataReady){alert('انتظر اكتمال تحميل البيانات أو اضغط تحديث إذا تعذر التحميل.');return;}
  const rows=getFilteredPendingOrders();
  if(!rows.length){alert('لا توجد بيانات للتصدير طبقًا للفلاتر الحالية');return;}
  if(typeof ExcelJS==='undefined'){alert('مكتبة Excel غير متاحة');return;}
  const actorId=String(currentUser.id);
  const branchFilter=isAdmin()||isOperationManager()?($('pendingBranchFilter')?.value||'all'):'الفروع المسموح بها';
  const branchLabel=branchFilter==='all'?'كل الفروع':branchFilter;
  const search=String($('pendingSearch')?.value||'').trim();
  pendingExportInProgress=true;
  if(button){button.disabled=true;button.setAttribute('aria-busy','true');}
  try{
    ...
    summary.addRows([
      ...
      ['أقدم أوردر — أيام بدون تحديث',rows.reduce((max,order)=>Math.max(max,pendingDays(order)),0)],
      ...PENDING_BRANCH_NAMES.filter(branch=>rows.some(order=>pendingOrderBranch(order)===branch)).map(branch=>[branch,rows.filter(order=>pendingOrderBranch(order)===branch).length])
    ]);
    const details=workbook.addWorksheet('Pending Orders');
    const headers=['#','Ticket ID','رقم الأوردر','العميل','الموبايل','الموبايل 2','الفرع','الموظف','الدكتور','الحالة','السعر','Deposit','المتبقي','أيام بدون تحديث','تاريخ الإنشاء','آخر تحديث','المنطقة','المنتجات','الملاحظات'];
    details.columns=[7,15,17,25,18,18,20,22,22,16,16,16,16,18,26,26,38,38,50].map(width=>({width}));
    details.addRow(headers);
    rows.forEach((order,index)=>details.addRow([
      index+1,String(getTicketId(order)||''),String(order.order_number||''),order.customer_name||'',String(order.phone||''),String(order.phone2||''),
      pendingOrderBranch(order),order.employee_name||'',order.doctor_name||'',order.status,
      getEffectiveOrderPrice(order),Number(order.deposit||0),getOrderOutstandingBalance(order),pendingDays(order),
      formatEnglishDateTime(order.created_at),formatEnglishDateTime(pendingReferenceDate(order)),order.area||'',order.product_names||'',cleanVisibleOrderNotes(order.notes||'')
    ]));
    [2,3,5,6].forEach(column=>details.getColumn(column).numFmt='@');
    [11,12,13].forEach(column=>details.getColumn(column).numFmt='#,##0.##');
    details.autoFilter={from:{row:1,column:1},to:{row:details.rowCount,column:headers.length}};
    ...
    if(!pendingDataReady||rows.some(order=>!availableIds.has(String(order.id))))throw new Error('تغيّرت قائمة Pending أثناء تجهيز الملف؛ أعد التصدير من النتائج الحالية.');
    const url=URL.createObjectURL(new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}));
    const link=document.createElement('a');link.href=url;link.download=`Pending-Orders-${getCairoDateISO()}.xlsx`;
    document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1200);
    logActivity('data_exported','تصدير Pending Orders',`الفرع: ${branchLabel} | البحث: ${search||'—'} | عدد الأوردرات: ${rows.length}`);
  }catch(error){alert('تعذر التصدير: '+(error?.message||error));}
  finally{pendingExportInProgress=false;if(button){button.disabled=!pendingDataReady;button.removeAttribute('aria-busy');}}
}
function renderPendingBranchSummary(){
  ...
  Object.entries(branchIds).forEach(([branch,id])=>{
    ...
    if(el)el.textContent=num(rows.filter(o=>pendingOrderBranch(o)===branch).length);
  });
}
...
function setupOKBStoresReportBranchFilter(){
  ...
  const branches=getOKBStoresReportBranches();
...
async function loadOKBStoresRescuedIds(signedOrders,isCurrent=()=>true){
  ...
  for(let start=0;start<signedOrders.length;start+=100){
    ...
    const tickets=[...new Set(batch.flatMap(order=>[getTicketId(order),storesReportTicketKey(getTicketId(order))]).filter(value=>/^\d+$/.test(value)))];
    const clauses=[];
    if(ids.length)clauses.push(`order_id.in.(${ids.join(',')})`);
    ...
    for(let offset=0;;offset+=1000){
      ...
      for(const event of data||[]){
        ...
        if(event.order_id!==null&&event.order_id!==undefined&&event.order_id!==''){
          ...
          order=byId.get(String(event.order_id));
        }else{
          // Legacy UUID orders have null BIGINT order_id in activity_logs.
          const matches=(byTicket.get(storesReportTicketKey(event.ticket_id))||[])
...
async function showOKBStoresReportPage(){if(!hasRoleFeature('stores_report')){alert('غير مسموح لك بفتح OKB Abnormal');return;}closeOKBStoresMenu();setOKBStoresReportCurrentMonth(false);const status=document.getElementById('storesReportStatus');if(status)status.value='Returned';hideAllPages();document.getElementById('okbStoresReportPage')?.classList.remove('hidden');setActiveMenu('okbStoresReportPage');setupOKBStoresReportBranchFilter();await loadOKBStoresReport(false);}
async function loadOKBStoresReport(showMessage=false){
  if(!currentUser||!hasRoleFeature('stores_report'))return;
  if(storesReportMonthKey)setOKBStoresReportCurrentMonth(false);
  const sequence=++storesReportLoadSequence,user=currentUser;
  ...
  try{
    ...
    if(branches.length)for(let offset=0;;offset+=1000){
      if(!isCurrent())return;
      let query=supabaseClient.from('orders').select('*').in('status',['Returned','Cancel','Signed']).in('branch',branches);
      ...
      if(error)throw error;
      for(const order of data||[])if(canSeeStoresReportOrder(order))ordersById.set(String(order.id),order);
      if(!data||data.length<1000)break;
    }
    if(!isCurrent())return;
    const candidates=[...ordersById.values()];
...
function getFilteredOKBStoresReportOrders(ignoreStatus=false){const q=String(document.getElementById('storesReportSearch')?.value||'').trim().toLowerCase(),status=document.getElementById('storesReportStatus')?.value||'Returned',branch=document.getElementById('storesReportBranch')?.value||'all',from=storesReportDateFrom,to=storesReportDateTo;return okbStoresReportOrders.filter(o=>{const d=parsePreciseServerDate(o.created_at),day=d?getCairoDateISO(d):'';return canSeeStoresReportOrder(o)&&(!from||day>=from)&&(!to||day<=to)&&(ignoreStatus||status==='all'||(status===STORES_REPORT_RESCUED_STATUS?isOKBStoresRescuedOrder(o):o.status===status))&&(branch==='all'||pendingOrderBranch(o)===branch)&&(!q||[o.customer_name,o.doctor_name,o.phone,o.phone2,o.order_number,getTicketId(o),o.employee_name,o.notes,pendingOrderBranch(o)].some(v=>String(v||'').toLowerCase().includes(q)));});}
function renderOKBStoresReport(){
  const list=document.getElementById('storesReportOrdersList');if(!list||!currentUser||!hasRoleFeature('stores_report'))return;const rows=getFilteredOKBStoresReportOrders();
  const branches=getOKBStoresReportBranches(),rescued=getFilteredOKBStoresReportOrders(true).filter(isOKBStoresRescuedOrder);
  const status=document.getElementById('storesReportStatus')?.value||'Returned';
  const awaitingHistory=(status==='all'||status===STORES_REPORT_RESCUED_STATUS)&&storesReportRescueLoading;
  const unavailable=storesReportLoading||!!storesReportLoadError||awaitingHistory||((status==='all'||status===STORES_REPORT_RESCUED_STATUS)&&!!storesReportRescueError);
  const count=value=>unavailable?'—':num(value);
  ...
  document.querySelector('#okbStoresReportPage .pending-summary')?.style.setProperty('--stores-report-summary-columns',String(4+branches.length));
  document.querySelectorAll('#okbStoresReportPage .stores-report-branch-card').forEach(card=>{card.style.display=branches.includes(card.dataset.storesReportBranch||'')?'flex':'none';});
  Object.entries(ids).forEach(([b,id])=>{const el=document.getElementById(id);if(el)el.textContent=count(rows.filter(o=>pendingOrderBranch(o)===b).length);});
  const rescueCounts=document.getElementById('storesReportRescuedCounts');
  if(rescueCounts)rescueCounts.innerHTML=branches.map(branch=>`<div><span>${escapeHTML(branch)}</span><strong>${storesReportRescueLoading||storesReportRescueError||storesReportLoadError?'—':num(rescued.filter(order=>pendingOrderBranch(order)===branch).length)}</strong></div>`).join('');
  const message=document.getElementById('storesReportRescuedMessage');
  if(message){message.textContent=storesReportRescueLoading?'جاري التحقق من سجل الحالات...':storesReportRescueError||(storesReportLoadError?'تعذر تحميل البيانات':'');message.classList.toggle('hidden',!message.textContent);}
  document.getElementById('storesReportTotalCount').textContent=count(rows.length);document.getElementById('storesReportReturnedCount').textContent=count(rows.filter(o=>o.status==='Returned').length);document.getElementById('storesReportCancelCount').textContent=count(rows.filter(o=>o.status==='Cancel').length);document.getElementById('storesReportTotalValue').textContent=unavailable?'—':money(rows.reduce((s,o)=>s+getEffectiveOrderPrice(o),0));
  if(storesReportLoadError){list.innerHTML=`<div class="pending-empty">تعذر تحميل التقرير: ${escapeHTML(storesReportLoadError)}</div>`;return;}
...
function openOKBStoresReportOrder(id){
  ...
  if(order)openReportOrderTab('stores_report',order);
}
...
function reportLinkedOrderBranch(order,source,activity){
  ...
  const raw=String(order?.branch||order?.shipping_company||activity?.branch_name||'').trim();
  const branch=ROLE_BRANCH_FEATURES[raw]?raw:(getBranchNameFromShippingCompany(raw)||getBranchNameFromShippingCompany(order?.shipping_company)||getBranchNameFromShippingCompany(activity?.branch_name)||'');
  return ROLE_BRANCH_FEATURES[branch]?branch:'';
}
function canOpenReportLinkedOrder(source,order,activity){
  if(!order||!canUseReportOrderLink(source))return false;
...
function openReportOrderTab(source,order,activityId=''){
  if(!canUseReportOrderLink(source)||(!order&&!(source==='activity_log'&&activityId))||(order&&!canOpenReportLinkedOrder(source,order))){alert('لا توجد صلاحية لفتح الأوردر من هذه الصفحة أو فرعه');return;}
  const params=source==='stores_report'?{okbAbnormalOrder:String(order.id)}:{okbOrderSource:source,...(source==='activity_log'?{okbActivityId:String(activityId)}:{okbOrderId:String(order.id)})};
  const url=new URL(window.location.href);url.hash=new URLSearchParams(params).toString();
  // Same-origin tabs inherit this app's sessionStorage. Sever the opener immediately
  // after opening; never put session tokens or customer details into the URL.
  const tab=window.open(url.href,'_blank');
  if(tab)tab.opener=null;else alert('المتصفح منع فتح التاب الجديدة. اسمح بالنوافذ المنبثقة لهذا الموقع ثم حاول مرة أخرى.');
}
async function openOKBAbnormalLinkedOrder(){
  const params=new URLSearchParams(window.location.hash.slice(1));
  const legacy=params.has('okbAbnormalOrder');
  if(!legacy&&!params.has('okbOrderSource'))return false;
  const source=legacy?'stores_report':params.get('okbOrderSource');
  const id=legacy?params.get('okbAbnormalOrder'):params.get('okbOrderId'),activityId=params.get('okbActivityId');
  ['okbAbnormalOrder','okbOrderSource','okbOrderId','okbActivityId'].forEach(key=>params.delete(key));
  const url=new URL(window.location.href);url.hash=params.toString();
  window.history.replaceState(window.history.state,'',url.href);
  try{
    if(!canUseReportOrderLink(source))throw new Error('لا توجد صلاحية لفتح الأوردر من هذه الصفحة');
    const validId=value=>/^(?:\d+|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/i.test(value||'');
    if(!validId(source==='activity_log'?activityId:id))throw new Error('رابط الأوردر غير صحيح');
    const user=currentUser;
    let order,activity;
    if(source==='activity_log'){
      const result=await supabaseClient.from('activity_logs').select('id,order_id,ticket_id,branch_name').eq('id',activityId).maybeSingle();
      if(result.error)throw result.error;activity=result.data;
      if(currentUser!==user||!canUseReportOrderLink(source))return false;
      if(!activity?.ticket_id)throw new Error('هذا النشاط غير موجود أو غير مرتبط بـ Ticket ID');
      if(validId(activity.order_id)){
        const result=await supabaseClient.from('orders').select('*').eq('id',activity.order_id).maybeSingle();
        if(result.error)throw result.error;order=result.data;
      }else{
        const digits=onlyDigits(activity.ticket_id),compact=digits.replace(/^0+(?=\d)/,'');
        if(!digits)throw new Error('Ticket ID غير صحيح');
        const tickets=[...new Set([digits,compact,compact.padStart(5,'0')])];
        const result=await supabaseClient.from('orders').select('*').or(`ticket_id.in.(${tickets.join(',')}),order_barcode.eq.${digits}`).limit(2);
        if(result.error)throw result.error;
        if(result.data?.length>1)throw new Error('Ticket ID مرتبط بأكثر من أوردر؛ يلزم مراجعة بياناته قبل الفتح');
        order=result.data?.[0];
      }
    }else{
      const result=await supabaseClient.from('orders').select('*').eq('id',id).maybeSingle();
      if(result.error)throw result.error;order=result.data;
    }
    if(currentUser!==user||!canOpenReportLinkedOrder(source,order,activity))throw new Error('الأوردر غير موجود أو لا توجد صلاحية لفتح صفحته');
    const branch=reportLinkedOrderBranch(order,source,activity),orderId=String(order.id);
    const stillAllowed=()=>currentUser===user&&canOpenReportLinkedOrder(source,order,activity);
    if(branch)await openBranchPage(branch);else await showOrdersPage();
    if(!stillAllowed())return false;
    let loaded=branch?branchOrders:getVisibleOrders();
    if(!loaded.some(item=>String(item.id)===orderId)){
      const day=getLocalDateISO(order.created_at);
      document.getElementById(branch?'bFromDate':'fromDate').value=day;document.getElementById(branch?'bToDate':'toDate').value=day;
      if(branch)await applyBranchDateFilter();else await applyDateFilter();
      loaded=branch?branchOrders:getVisibleOrders();
    }
    if(!stillAllowed())return false;
    if(!loaded.some(item=>String(item.id)===orderId))throw new Error('تعذر تحميل الأوردر في صفحته. أعد المحاولة.');
    const filters=branch?['bFilterStatus','bFilterEmployee','bFilterDoctor']:['filterStatus','filterEmployee','filterDoctor'];
    filters.forEach(key=>{const el=document.getElementById(key);if(el){el.value='الكل';searchableOrderSelects.get(key)?.syncFromSelect();}});
    const search=document.getElementById(branch?'bSearchInput':'searchInput');if(search)search.value=getTicketId(order);
    if(branch){branchPageNum=1;renderBranchOrders();}else{pageState.orders=1;renderOrders();}
    search?.focus();
    return true;
  }catch(error){alert(error.message||'تعذر فتح الأوردر');return false;}
}
async function exportOKBStoresReportExcel(){
  if(!currentUser||!hasRoleFeature('stores_report')){alert('لا توجد صلاحية لتصدير OKB Abnormal');return;}
  const status=document.getElementById('storesReportStatus')?.value||'Returned';
...
async function showInitialPermittedPage() {
  if (await openOKBAbnormalLinkedOrder()) return;
...
function resetForm() { 
  ...
  const errorOrderEl = document.getElementById('dashErrorOrder');
  if (urgentEl) urgentEl.checked = false;
  if (replacementEl) replacementEl.checked = false;
  if (errorOrderEl) errorOrderEl.checked = false;
...
function cartItemsQuantity(scope){
  return getCartArray(scope).reduce((sum,p)=>sum+Math.max(0,Number(p.qty||0)),0);
}

function applyDiscountToGrandTotal(scope){
  const prefix=cartPrefix(scope);
  const totalInput=document.getElementById(prefix+'GrandTotal');
  if(isAdmin()&&totalInput?.dataset?.adminOverride!==undefined){
    delete totalInput.dataset.adminOverride;
  }
...
function syncProductCartTotals(scope) {
  ...
  if(scope==='branch'){
    const fixedProductsTotal=document.getElementById('branchProductsFixedTotal');
    const fixedItemsQty=document.getElementById('branchItemsFixedQty');
    if(fixedProductsTotal)fixedProductsTotal.textContent=money(productsTotal);
    if(fixedItemsQty)fixedItemsQty.textContent=num(cartItemsQuantity(scope));
  }
...
function renderOrderDoctorFilters(){
  const masterDoctors=(doctorsList||[]).map(doctor=>({name:String(doctor?.name||'').trim(),code:String(doctor?.code||doctor?.doctor_code||'').trim()})).filter(doctor=>doctor.name).sort((a,b)=>a.name.localeCompare(b.name,'ar'));
  const options=masterDoctors.map(doctor=>`<option value="${escapeHTML(doctor.name)}">${escapeHTML(doctor.code?`${doctor.name} - ${doctor.code}`:doctor.name)}</option>`).join('');
  ['filterDoctor','bFilterDoctor'].forEach(id=>{
    const select=document.getElementById(id);if(!select)return;
    const current=select.value||'الكل';
    select.innerHTML=`<option value="الكل">كل الدكاترة</option>${options}`;
    select.value=masterDoctors.some(doctor=>doctor.name===current)?current:'الكل';
  });
  refreshOrderSearchableSelects();
}
...
function getFilteredOrders() {
  ...
  const doctorFilter=document.getElementById('filterDoctor')?.value||'الكل';
  return getVisibleOrders().filter(o => {
    ...
    // month. Date filters remain active when the search box is empty.
    const matchDate = search ? true : isInDateRange(o);
    return matchSearch && matchStatus && matchEmployee && matchShipping && matchDoctor && matchDate;
  });
}

function getHistoricalSearchArray(scope){
  if(scope==='branch')return branchOrders;
  if(scope==='khazna')return khaznaOrders;
  return orders;
}
function setHistoricalSearchArray(scope,value){
  if(scope==='branch')branchOrders=value;
  else if(scope==='khazna')khaznaOrders=value;
  else orders=value;
}
function clearHistoricalSearchRows(scope){
  const state=historicalOrderSearchState[scope];if(!state?.addedIds?.size)return;
  setHistoricalSearchArray(scope,getHistoricalSearchArray(scope).filter(order=>!state.addedIds.has(String(order.id))));
  state.addedIds.clear();
}
function renderHistoricalSearchScope(scope){
  if(scope==='branch'){branchPageNum=1;renderBranchOrders();return;}
  if(scope==='khazna'){renderKhaznaStats();renderKhaznaOrders();return;}
  pageState.orders=1;renderOrders();
}
async function runHistoricalOrderSearch(scope,rawValue){
  const state=historicalOrderSearchState[scope];if(!state)return [];
  const token=++state.token;
  clearTimeout(state.timer);state.timer=null;
  clearHistoricalSearchRows(scope);
  const raw=String(rawValue||'').trim();
  if(!raw){renderHistoricalSearchScope(scope);return [];}
  const term=raw.replace(/[,%()'"\\]/g,' ').replace(/\s+/g,' ').trim();
  if(!term){renderHistoricalSearchScope(scope);return [];}
  const fields=['customer_name','phone','phone2','doctor_name','employee_name','order_number','ticket_id','order_barcode'];
  let query=supabaseClient.from('orders').select('*').or(fields.map(field=>`${field}.ilike.%${term}%`).join(',')).order('created_at',{ascending:false}).limit(300);
  if(scope==='branch'||scope==='khazna')query=query.eq('branch',currentBranchName);
  const {data,error}=await query;
  if(token!==state.token)return [];
  if(error){console.warn('Historical order search:',error.message);renderHistoricalSearchScope(scope);return [];}
  let rows=Array.isArray(data)?data:[];
  if(scope==='dashboard'&&isAccountSupervisor())rows=rows.filter(order=>isOrderInManagedBranches(order));
  const target=getHistoricalSearchArray(scope),existing=new Set(target.map(order=>String(order.id)));
  rows.forEach(order=>{const id=String(order.id);if(existing.has(id))return;target.push(order);existing.add(id);state.addedIds.add(id);});
  target.sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));
  renderHistoricalSearchScope(scope);
  return rows;
}
function scheduleHistoricalOrderSearch(scope,value){
  const state=historicalOrderSearchState[scope];if(!state)return;
  clearTimeout(state.timer);
  state.token++;
  if(!String(value||'').trim()){void runHistoricalOrderSearch(scope,'');return;}
  state.timer=setTimeout(()=>void runHistoricalOrderSearch(scope,value),350);
}
...
orderForm.addEventListener("submit", async (e) => {
  ...
  let depositValue = depositEl ? Number(depositEl.value) || 0 : 0;
  const errorOrderSelected = Boolean(document.getElementById('dashErrorOrder')?.checked);
  const replacementOrderSelected = Boolean(document.getElementById('dashReplacementOrder')?.checked);
...
window.editOrder = function (id) {
  ...
  if (errorOrderEl) errorOrderEl.checked = orderFlags.errorOrder;
...
function getOperationReportOrderDay(order) {
  const date=parsePreciseServerDate(order?.created_at);
  return date?getCairoDateISO(date):'';
}
...
function getOperationReportOrders(from,to,branchKeys,sourceOrders=orders) {
  ...
  return (sourceOrders||[]).filter(order=>{
    ...
    const day=getOperationReportOrderDay(order);
...
async function fetchOperationReportOrders(from,to,branchKeys) {
  const range=getCairoDateRangeUTC(from,to);
  if(!range)throw new Error('اختر فترة تاريخ صحيحة للتقرير');
  const pageSize=1000;
  let offset=0;
  let result=[];
  while(true){
  ...
  return getOperationReportOrders(from,to,branchKeys,result);
}

function getOperationReportMetrics(list) {
  // The central financial analysis supplies every value; Branch Rank supplies
  // the established countable-order and operational Total rules.
  const eligible=(Array.isArray(list)?list:[]).map(analyzeOrderFinancials)
    .filter(item=>normalizeOrderPriceForStorage(item.price)>0);
  const stats=calculateBranchMiniDashboardStats(eligible.map(item=>item.order),{excludeGroupCancelFromTotal:true});
  const source=eligible.filter(item=>item.status!=='Cancel');
  const signed=source.filter(item=>item.status==='Signed');
  const delivering=source.filter(item=>item.status==='Delivering');
  const returned=source.filter(item=>item.status==='Returned');
  const total=stats.total;
  const revenue=source.reduce((sum,item)=>sum+item.price,0);
  const signedRevenue=signed.reduce((sum,item)=>sum+item.price,0);
  const returnedValue=returned.reduce((sum,item)=>sum+item.price,0);
  const deliveringValue=delivering.reduce((sum,item)=>sum+item.price,0);
  return {
    total,signed:stats.signed,delivering:stats.delivering,
    returned:stats.returned,cancelled:stats.cancelled,revenue,signedRevenue,returnedValue,deliveringValue,
    averageOrder:total?revenue/total:0,
    conversionRate:total?signed.length/total:0,
    returnRate:total?returned.length/total:0,
    cancelRate:stats.rawTotal?stats.cancelled/stats.rawTotal:0
  };
}
...
function getOperationReportWeeks(from,to) {
  if(!operationReportDate(from)||!operationReportDate(to)||from>to)return [];
  const periods=[];
  const multipleMonths=from.slice(0,7)!==to.slice(0,7);
  let monthStart=from.slice(0,7)+'-01';
  while(monthStart<=to){
    ...
    [1,8,15,22].forEach((day,index)=>{
      const periodFrom=month+'-'+String(day).padStart(2,'0');
      const periodTo=index===3?monthEnd:month+'-'+String(day+6).padStart(2,'0');
      const clippedFrom=periodFrom<from?from:periodFrom;
      const clippedTo=periodTo>to?to:periodTo;
      if(clippedFrom<=clippedTo)periods.push({label:`Week ${index+1}${multipleMonths?' — '+month:''}`,from:clippedFrom,to:clippedTo});
    });
    monthStart=shiftISODate(monthEnd,1);
  }
  return periods;
}

function getOperationReportWeeklyMetrics(list,from,to,current=getOperationReportMetrics(list)) {
  let covered=0;
  const dated=list.map(order=>({order,day:getOperationReportOrderDay(order)}));
  const weeks=getOperationReportWeeks(from,to).map(period=>{
    const rows=dated.filter(item=>item.day&&item.day>=period.from&&item.day<=period.to).map(item=>item.order);
    covered+=rows.length;
    return {...period,metrics:getOperationReportMetrics(rows)};
  });
  const fields=['total','signed','delivering','returned','cancelled','revenue','signedRevenue','returnedValue','deliveringValue'];
  const mismatch=fields.some(key=>{
    const sum=weeks.reduce((total,week)=>total+week.metrics[key],0);
    return !Number.isFinite(sum)||!Number.isFinite(current[key])||Math.abs(sum-current[key])>FINANCIAL_TOLERANCE;
  });
  if(covered!==list.length||mismatch)throw new Error('بيانات الأسابيع لا تطابق إجمالي الفرع؛ لم يتم تصدير التقرير. حدّث البيانات وحاول مرة أخرى.');
  return weeks;
}
...
function addOperationBranchSheet(workbook,branch,currentOrders,previousOrders,from,to,previousFrom,previousTo) {
  ...
  const weeks=getOperationReportWeeklyMetrics(currentOrders,from,to,current);
  ...
  weeks.forEach(period=>{
    ...
    sheet.addRow([period.label,period.from,period.to,metrics.total,metrics.signed,metrics.delivering,metrics.returned,metrics.cancelled,metrics.conversionRate,metrics.returnRate,metrics.revenue]);
  });
  [8,9].forEach(row=>[2,3,4,5].forEach(col=>sheet.getCell(row,col).numFmt='0.0%'));
  [3,4,5,6,7,10,11].forEach(row=>sheet.getCell(row,4).numFmt='0.0%');
  [10,11].forEach(row=>[2,3].forEach(col=>sheet.getCell(row,col).numFmt='#,##0.00'));
  for(let row=14;row<=sheet.rowCount;row+=1){sheet.getCell(row,9).numFmt='0.0%';sheet.getCell(row,10).numFmt='0.0%';sheet.getCell(row,11).numFmt='#,##0.00';}
  mergeOperationManagerTitle(sheet,5,`${branch.name} — Monthly Operations`);
  styleOperationManagerSheet(sheet,[2,13]);
...
async function exportOperationManagerReport(button) {
  ...
  const actorId=String(currentUser.id);
  const stillAllowed=()=>String(currentUser?.id||'')===actorId&&hasRoleFeature('operation_manager_report')&&branches.every(branch=>getOperationReportAllowedBranches().some(allowed=>allowed.key===branch.key));
  ...
  try{
    ...
    if(!stillAllowed())throw new Error('تغيّرت صلاحيات التقرير؛ حدّث الصفحة قبل التصدير.');
    ...
    [12,13,14].forEach(row=>[2,3,4,5].forEach(col=>overview.getCell(row,col).numFmt='0.0%'));
    [7,8,9,10,11,15,16,17,18,19,20].forEach(row=>overview.getCell(row,4).numFmt='0.0%');
    [15,16,17,18,19,20].forEach(row=>[2,3].forEach(col=>overview.getCell(row,col).numFmt='#,##0.00'));
    overview.getCell('B20').numFmt='#,##0.00';
    styleOperationManagerSheet(overview,[6]);
    ['A8','B8','A15','B15','A16','B16','A20','B20'].forEach(ref=>{overview.getCell(ref).font={name:'Calibri',size:11,bold:true,color:{argb:'FF047857'}};});
    ['A10','B10','A11','B11','A18','B18'].forEach(ref=>{overview.getCell(ref).font={name:'Calibri',size:11,bold:true,color:{argb:'FFDC2626'}};});

    const comparison=workbook.addWorksheet('Branch Comparison');
    comparison.columns=[22,13,13,13,13,15,14,16,16,17,17,17].map(width=>({width}));
    comparison.addRow([`Branch Comparison — ${from} → ${to}`]);comparison.addRow([]);
    comparison.addRow(['Branch','Total','Signed','Delivering','Returned','Group Cancel','Conversion Rate','Return Rate','Revenue','Previous Conversion','Previous Return','Revenue Change']);
    branchResults.forEach(item=>comparison.addRow([item.branch.name,item.current.total,item.current.signed,item.current.delivering,item.current.returned,item.current.cancelled,item.current.conversionRate,item.current.returnRate,item.current.revenue,item.previous.conversionRate,item.previous.returnRate,operationReportChange(item.current.revenue,item.previous.revenue)]));
    for(let row=4;row<=comparison.rowCount;row+=1){[7,8,10,11,12].forEach(col=>comparison.getCell(row,col).numFmt='0.0%');comparison.getCell(row,9).numFmt='#,##0.00';}
    comparison.autoFilter={from:{row:3,column:1},to:{row:3,column:12}};mergeOperationManagerTitle(comparison,12,`Branch Comparison — ${from} → ${to}`);styleOperationManagerSheet(comparison,[3]);
    ...
    finance.addRows([
      ...
      ['Total Revenue',currentTotal.revenue,previousTotal.revenue,operationReportChange(currentTotal.revenue,previousTotal.revenue),'قيمة الأوردرات الفعلية بدون Cancel والأوردرات الصفرية'],
...
function renderUsers() {
  ...
  usersTableBody.innerHTML = visibleUsers.map(u => {
...
// ===== مستمعي الأحداث =====
searchInput.addEventListener("input", () => { pageState.orders = 1; renderOrders(); scheduleHistoricalOrderSearch('dashboard',searchInput.value); });
searchInput.addEventListener("keydown", async (ev) => {
  if (ev.key !== "Enter") return;
  ev.preventDefault();
  const scannedValue = String(searchInput.value || "").trim();
  if(scannedValue)await runHistoricalOrderSearch('dashboard',scannedValue);
...
const filterDoctorEl=document.getElementById('filterDoctor');
if(filterDoctorEl)filterDoctorEl.addEventListener('change',()=>{pageState.orders=1;renderOrders();});
...
function getBranchFilteredOrders() {
  ...
  const doctorFilter=document.getElementById('bFilterDoctor')?.value||'الكل';
  ...
  return branchOrders.filter(o => {
    ...
    return matchSearch && matchStatus && matchEmp && matchDoctor && matchDate;
  });
}
...
function appendVisibleOrderNote(notes, addition) {
  ...
  const withoutCollect = original.replace(COLLECT_META_REGEX, '').trim();
  const orderMetaMatch = withoutCollect.match(ORDER_META_REGEX);
...
function getBranchStatusButtonHtml(order) {
  const status=String(order?.status||'').trim().toLowerCase();
  if (status === 'signed') return `<span class="branch-cancel-disabled branch-delivered-label" title="تم تسليم الأوردر">تم التسليم</span>`;
...
function canUseBranchInlineEdit(order, showMessage = false) {
  ...
  if (String(order.status || '').trim().toLowerCase() === 'signed') return deny('تعديل الأوردر المسلم متاح للأدمن من التحديد الذكي فقط.');
  if (isAdmin()) return true;
...
async function openBranchShippingRankFromBranch() {
  ...
  rememberAppPage('shippingRankPage');
  const backBtn = document.getElementById('branchShippingBackBtn');
...
function renderBranchOrders() {
  ...
  rows.forEach((o, i) => {
    ...
    html += `<tr class="${getOrderPriorityRowClass(o)}">
...
let dashboardNoteOrderId='';
let customerProfileNoteOrderId='';

function closeDashboardActions(){
  document.querySelectorAll('.dashboard-actions-dropdown').forEach(menu=>menu.classList.add('hidden'));
  document.querySelectorAll('.dashboard-actions-trigger').forEach(button=>button.setAttribute('aria-expanded','false'));
}

function toggleDashboardActions(event,id){
  event?.stopPropagation();
  const menu=document.getElementById(`dashboardActions_${id}`);
  if(!menu)return;
  ...
  if(willOpen){
    ...
    if(rect){
      ...
      const top=rect.bottom+5+menuHeight>window.innerHeight ? Math.max(8,rect.top-menuHeight-5) : rect.bottom+5;
      menu.style.left=`${left}px`;
...
document.addEventListener('click',event=>{
  if(!event.target.closest('.dashboard-actions-menu'))closeDashboardActions();
});
window.addEventListener('resize',closeDashboardActions);
window.addEventListener('scroll',closeDashboardActions,true);
...
function openSelectedBranchNoteModal(){
  dashboardNoteOrderId='';
  customerProfileNoteOrderId='';
...
function openDashboardOrderNote(id){
  const order=orders.find(item=>String(item.id)===String(id));
  if(!order){alert('تعذر العثور على الأوردر');return;}
  dashboardNoteOrderId=String(id);
  customerProfileNoteOrderId='';
  const modal=ensureBranchBulkNoteModal();
  const scope=document.getElementById('branchBulkNoteScope');
  const input=document.getElementById('branchBulkNoteInput');
  if(scope)scope.textContent=`إضافة ملاحظة على أوردر العميل ${order.customer_name||'—'}، أسفل الملاحظات السابقة.`;
  if(input)input.value='';
  modal.classList.remove('hidden');
  setTimeout(()=>input?.focus(),30);
}

function openCustomerProfileOrderNote(){
  const order=customerProfileOrders.find(item=>String(item.id)===String(customerProfileFocusOrderId))||customerProfileOrders[0];
  if(!order){alert('تعذر العثور على أوردر متاح لإضافة الملاحظة');return;}
  customerProfileNoteOrderId=String(order.id);
  dashboardNoteOrderId='';
  const modal=ensureBranchBulkNoteModal();
  const scope=document.getElementById('branchBulkNoteScope');
  const input=document.getElementById('branchBulkNoteInput');
  if(scope)scope.textContent=`إضافة ملاحظة على أوردر العميل ${order.customer_name||'—'} — Ticket ${getTicketId(order)||'—'}، أسفل الملاحظات السابقة.`;
  if(input)input.value='';
  modal.classList.remove('hidden');
  setTimeout(()=>input?.focus(),30);
}
...
async function saveSelectedBranchNote(){
  ...
  const isCustomerProfileNote=Boolean(customerProfileNoteOrderId);
  const isDashboardNote=!isCustomerProfileNote&&Boolean(dashboardNoteOrderId);
  const selected=isCustomerProfileNote
    ? customerProfileOrders.filter(order=>String(order.id)===String(customerProfileNoteOrderId))
    : isDashboardNote
  ...
  if(!selected.length){alert('الأوردرات المحددة غير موجودة');closeSelectedBranchNoteModal();if(!isCustomerProfileNote&&!isDashboardNote)clearBranchOrderSelection();return;}
  ...
  for(const order of selected){
    ...
    [orders,branchOrders,khaznaOrders,branchesTreasuryOrders,pendingOrders,customerProfileOrders].forEach(list=>{
      const cached=list.find(item=>String(item.id)===String(order.id));
      if(cached)cached.notes=updatedNotes;
    });
  ...
  if(savedOrders.length){
    await Promise.all(savedOrders.map(order=>logActivity(
      'order_note_added',
      'إضافة ملاحظة على أوردر',
      `العميل: ${order.customer_name||'—'} | الملاحظة: ${note}`,
  ...
  dashboardNoteOrderId='';
  customerProfileNoteOrderId='';
  if(isCustomerProfileNote){renderCustomerProfile();if(isLivePageVisible('ordersPage'))renderOrders();if(isLivePageVisible('branchPage'))renderBranchOrders();}
  else if(isDashboardNote)renderOrders();
  else{
...
async function resetBranchDateFilter() {
  ...
  const doctor=document.getElementById('bFilterDoctor');if(doctor)doctor.value='الكل';
  const badge = document.getElementById('bActiveDateBadge');
  branchPageNum = 1;
...
function editBranchOrder(id) {
  ...
  const errorOrderEl = document.getElementById('branchErrorOrder');
  if (urgentEl) urgentEl.checked = orderFlags.urgent;
  if (replacementEl) replacementEl.checked = orderFlags.replacement;
  if (errorOrderEl) errorOrderEl.checked = orderFlags.errorOrder;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (bSearch) {
    bSearch.addEventListener('input', () => { branchPageNum = 1; renderBranchOrders(); scheduleHistoricalOrderSearch('branch',bSearch.value); });
    bSearch.addEventListener('keydown', async (ev) => {
      if (ev.key === 'Enter') {
        ev.preventDefault();
        branchPageNum = 1;
        const scannedValue = String(bSearch.value || '').trim();
        if(scannedValue)await runHistoricalOrderSearch('branch',scannedValue);
        renderBranchOrders();
        if (scannedValue) openCollectionFromScan(scannedValue, 'branch');
      }
    });
  ...
  const bFDoctor=document.getElementById('bFilterDoctor');
  if(bFDoctor)bFDoctor.addEventListener('change',()=>{branchPageNum=1;renderBranchOrders();});

  const khaznaSearch = document.getElementById('khaznaBarcodeSearch');
  if (khaznaSearch) {
    khaznaSearch.addEventListener('input', () => { renderKhaznaStats(); renderKhaznaOrders(); scheduleHistoricalOrderSearch('khazna',khaznaSearch.value); });
    khaznaSearch.addEventListener('keydown', async (ev) => {
      if (ev.key !== 'Enter') return;
      ev.preventDefault();
      const scannedValue = String(khaznaSearch.value || '').trim();
      if(scannedValue)await runHistoricalOrderSearch('khazna',scannedValue);
...
async function loadKhaznaShippingOverride(from, to) {
  // إجمالي مصروفات الشحنات مصدره الوحيد هو مصروف كل شحنة المسجل أثناء
  // التحصيل. لا نحمّل أي تصحيح يدوي قديم حتى تظل الكروت والتقارير والقفل
  // اليومي على نفس المصدر المحاسبي المركزي.
  khaznaShippingCost = null;
}
...
function analyzeOrderFinancials(order) {
  ...
  const secretaryTransfer = !courtesyPrepaidOrder && deposit > 0 && upfrontProof ? deposit : 0;
  ...
  if (status === 'Signed') {
    ...
    if (!courtesyPrepaidOrder && upfrontProof && deposit <= 0) errors.push('إثبات سكرتارية موجود والمبلغ صفر');
...
function openFinancialAuditOrder(id) {
  ...
  if (!order) return;
  openReportOrderTab('financial_audit',order);
}
...
function parseSecretaryAuditEventInfo(event){
...
function scheduleSecretaryAuditLiveRefresh(){
  ...
  secretaryAuditLiveRefreshTimer=setTimeout(async()=>{
    if(!document.getElementById('secretaryAuditPage')?.classList.contains('hidden')){await refreshSecretaryAudit(false);await markSecretaryAuditRead();}
  },350);
}
async function handleSecretaryAuditOcrNotification(payload){
...
function startSecretaryAuditNotifications(){
  ...
  secretaryAuditRealtimeChannel=supabaseClient.channel(`secretary-audit-${currentUser?.id||Date.now()}`).on('postgres_changes',{event:'INSERT',schema:'public',table:'activity_logs'},payload=>{
    const action=String(payload?.new?.action_type||'');
    if(!SECRETARY_AUDIT_NOTIFICATION_ACTIONS.includes(action)||!isFourBranchSecretaryAuditEvent(payload?.new))return;
    const open=!document.getElementById('secretaryAuditPage')?.classList.contains('hidden');
    if(open)scheduleSecretaryAuditLiveRefresh();
    else updateSecretaryAuditBadge(secretaryAuditUnreadCount+1);
  }).on('postgres_changes',{event:'*',schema:'public',table:'payment_ocr_audits'},payload=>{
    void handleSecretaryAuditOcrNotification(payload);
  }).subscribe();
...
function analyzeSecretaryOrderInput(order) {
  ...
  if (!courtesyPrepaidOrder && deposit > FINANCIAL_TOLERANCE && !upfrontProof) errors.push(`تحويل سكرتارية ${enMoney(deposit)} بدون إثبات دفع`);
  if (!courtesyPrepaidOrder && upfrontProof && deposit <= FINANCIAL_TOLERANCE) warnings.push('إثبات سكرتارية موجود بدون مبلغ Deposit');
  if (!courtesyPrepaidOrder && Number.isFinite(price) && price < 50 && !visibleNotes) errors.push('أوردر أقل من 50 بدون توضيح هل هو استبدال أم هدية في الملاحظات');
  try{
...
function buildSecretaryAuditFindings(rows) {
  ...
  (rows||[]).forEach(order=>{
    ...
    const add=(severity,issue)=>findings.push({severity,issue,order,ticket:getTicketId(order)||order.order_number||'—',branch:getSecretaryAuditOrderBranch(order)||order.branch||'—',actor:order.employee_name||'—',created_at:order.updated_at||order.created_at||order.date||''});
...
function normalizeSecretaryAuditIdentity(value){
  return String(value||'').trim().toLowerCase().replace(/[\s\-_]+/g,' ');
}
function findSecretaryAuditCorrectedOrder(info,event,rows){
  const branch=getAuditChatBranch({shipping_company:info.shipping_company||'',branch:info.branch||event?.branch_name||''});
  if(!branch)return null;
  const eventTime=new Date(event?.created_at||0).getTime();
  const phones=[normalizeCustomerPhoneClient(info.phone),normalizeCustomerPhoneClient(info.phone2)].filter(phone=>phone.length>=10);
  const customer=normalizeSecretaryAuditIdentity(info.customer_name||event?.customer_name);
  const actor=normalizeSecretaryAuditIdentity(info.actor||event?.user_name||event?.username);
  return (rows||[]).map(order=>{
    if(getSecretaryAuditOrderBranch(order)!==branch)return null;
    const orderTime=new Date(order.created_at||order.updated_at||0).getTime();
    if(Number.isFinite(eventTime)&&eventTime>0&&Number.isFinite(orderTime)&&orderTime+5000<eventTime)return null;
    const orderPhones=[normalizeCustomerPhoneClient(order.phone),normalizeCustomerPhoneClient(order.phone2)].filter(phone=>phone.length>=10);
    const phoneMatch=phones.some(phone=>orderPhones.includes(phone));
    const customerMatch=Boolean(customer&&customer===normalizeSecretaryAuditIdentity(order.customer_name));
    const actorMatch=Boolean(actor&&actor===normalizeSecretaryAuditIdentity(order.employee_name));
    if(!phoneMatch&&!(customerMatch&&actorMatch))return null;
    return {order,score:(phoneMatch?100:0)+(customerMatch?20:0)+(actorMatch?10:0),time:Number.isFinite(orderTime)?orderTime:Number.MAX_SAFE_INTEGER};
...
async function loadSecretaryAuditRows() {
  ...
  const to=document.getElementById('secretaryAuditTo')?.value||today;
  if(from>to)throw new Error('تاريخ البداية يجب أن يكون قبل تاريخ النهاية');
  const utcRange=getCairoDateRangeUTC(from,to);
  if(!utcRange)throw new Error('تعذر تحديد نطاق التاريخ بتوقيت القاهرة');
  const fromInput=document.getElementById('secretaryAuditFrom'),toInput=document.getElementById('secretaryAuditTo');
  if(fromInput&&!fromInput.value)fromInput.value=today;if(toInput&&!toInput.value)toInput.value=today;
  let rows=[],offset=0;
  while(true){
    const result=await supabaseClient.from('orders').select('*').gte('created_at',utcRange.start).lt('created_at',utcRange.endExclusive).order('created_at',{ascending:false}).range(offset,offset+999);
    if(result.error)throw result.error;
...
function closeSecretaryAudit(){showInitialPermittedPage();}
...
async function resetSecretaryAuditFilters(){
  ...
  await refreshSecretaryAudit();
}
function getFilteredSecretaryAuditFindings(){
  const severity=document.getElementById('secretaryAuditSeverityFilter')?.value||'all';
  const branch=document.getElementById('secretaryAuditBranchFilter')?.value||'all';
  const employee=document.getElementById('secretaryAuditEmployeeFilter')?.value||'all';
  const query=normalizeSearchableText(document.getElementById('secretaryAuditSearch')?.value||'');
  return secretaryAuditState.findings.filter(item=>{
    if(!((severity==='all'||item.severity===severity)&&(branch==='all'||String(item.branch||'').includes(branch))&&(employee==='all'||String(item.actor||'')===employee)))return false;
    if(!query)return true;
    const order=item.order||{};
    return [item.ticket,item.issue,item.actor,item.branch,order.customer_name,order.phone,order.phone2,order.order_number,order.doctor_name,order.status].some(value=>normalizeSearchableText(value).includes(query));
  });
}
function populateSecretaryAuditEmployeeFilter(){
  const select=document.getElementById('secretaryAuditEmployeeFilter');if(!select)return;
  const current=select.value||'all';
  const names=[...new Set([...(Array.isArray(users)?users.map(user=>user?.name||user?.username):[]),...secretaryAuditState.rows.map(row=>row?.employee_name),...secretaryAuditState.findings.map(item=>item?.actor)].map(value=>String(value||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ar'));
  select.innerHTML='<option value="all">كل الموظفين</option>'+names.map(name=>`<option value="${escapeHTML(name)}">${escapeHTML(name)}</option>`).join('');
  select.value=names.includes(current)?current:'all';
  searchableOrderSelects.get('secretaryAuditEmployeeFilter')?.syncFromSelect();
}
function getSecretaryAuditOrderBranch(order){
  return getAuditChatBranch(order);
}
function getSecretaryAuditBranchOrders(){return secretaryAuditState.rows.filter(order=>PENDING_BRANCH_NAMES.includes(getSecretaryAuditOrderBranch(order)));}
function getSecretaryAuditTopEmployee(findings){
...
async function toggleSecretaryAuditReport(){
  ...
  const result=await supabaseClient.from('activity_logs').select('id,order_id,action_details').in('action_type',SECRETARY_AUDIT_ACTIONS).gte('created_at',start.toISOString()).lte('created_at',end.toISOString()).limit(2000);
  if(result.error)secretaryAuditWeeklyCount=0;
  else{
    const events=(result.data||[]).map(event=>{let info={};try{info=JSON.parse(event.action_details||'{}');}catch(_){ }return {...event,resolved_order_id:String(info.order_id||event.order_id||'').trim()};});
    const orderIds=[...new Set(events.map(event=>event.resolved_order_id).filter(Boolean))];
    let existingIds=new Set();
    if(orderIds.length){const existing=await supabaseClient.from('orders').select('id').in('id',orderIds);if(!existing.error)existingIds=new Set((existing.data||[]).map(row=>String(row.id)));}
    secretaryAuditWeeklyCount=events.filter(event=>!event.resolved_order_id||existingIds.has(event.resolved_order_id)).length;
  }
  renderSecretaryAuditReport();
}
function renderSecretaryAuditReport(){
  const panel=document.getElementById('secretaryAuditReportPanel');if(!panel)return;
  const current=secretaryAuditState.findings.filter(x=>x.severity!=='resolved'),top=getSecretaryAuditTopEmployee(current),issueCounts={},branchCounts={};
...
async function exportSecretaryAudit(){
  ...
  const exportedFindings=getFilteredSecretaryAuditFindings();
  const errors=exportedFindings.filter(item=>item.severity==='error').length;
  const warnings=exportedFindings.filter(item=>item.severity==='warning').length;
  const resolved=exportedFindings.filter(item=>item.severity==='resolved').length;
  const affectedOrders=new Set(exportedFindings.map(item=>String(item.order?.id||item.ticket||''))).size;
  const overview=[
    ['Secretary Audit Report',`${state.from||'—'} → ${state.to||'—'}`],
    ['Total Orders',getSecretaryAuditBranchOrders().length],
  ...
  const details=exportedFindings.map((item,index)=>{const order=item.order||{};return [index+1,item.severity==='error'?'Error':item.severity==='resolved'?'Resolved':'Warning',item.ticket||getTicketId(order)||'',order.order_number||'',order.customer_name||'',order.doctor_name||'',item.actor||order.employee_name||'',item.branch||pendingOrderBranch(order)||'',order.phone||'',Number(getEffectiveOrderPrice(order)||0),Number(order.deposit||0),order.status||'',formatEnglishDateTime(order.created_at||order.date||''),item.issue||''];});
  const workbook=XLSX.utils.book_new();
  ...
  try{logActivity('data_exported',`تم تصدير Secretary Audit للفترة ${safeFrom} → ${safeTo} | ${exportedFindings.length} نتيجة`);}catch(_){ }
}

function secretaryAuditDecision(errorRate){
  if(errorRate<=2)return 'أداء ممتاز — استمرار ومتابعة دورية';
  if(errorRate<=5)return 'أداء جيد — يحتاج تنبيه بسيط';
  if(errorRate<=10)return 'يحتاج خطة تحسين ومتابعة أسبوعية';
  return 'مراجعة إدارية عاجلة وتدريب إلزامي';
}
async function exportSecretaryAuditManagementReport(){
  if(!hasRoleFeature('secretary_audit')){alert('Secretary Audit غير مضاف لصلاحيات حسابك');return;}
  if(typeof ExcelJS==='undefined'){alert('مكتبة Excel غير متاحة حاليًا. برجاء إعادة تحميل الصفحة والمحاولة مرة أخرى.');return;}
  if(!await refreshSecretaryAudit(false))return;
  const branchFilter=document.getElementById('secretaryAuditBranchFilter')?.value||'all';
  const employeeFilter=document.getElementById('secretaryAuditEmployeeFilter')?.value||'all';
  const severityFilter=document.getElementById('secretaryAuditSeverityFilter')?.value||'all';
  const allOrders=getSecretaryAuditBranchOrders().filter(order=>(branchFilter==='all'||getSecretaryAuditOrderBranch(order)===branchFilter)&&(employeeFilter==='all'||String(order.employee_name||'')===employeeFilter));
  const findings=secretaryAuditState.findings.filter(item=>PENDING_BRANCH_NAMES.includes(String(item.branch||''))&&(branchFilter==='all'||item.branch===branchFilter)&&(employeeFilter==='all'||String(item.actor||'')===employeeFilter)&&(severityFilter==='all'||item.severity===severityFilter));
  const branches=branchFilter==='all'?PENDING_BRANCH_NAMES:[branchFilter];
  const workbook=new ExcelJS.Workbook();workbook.creator='OKB CRM';workbook.created=new Date();
  const headerFill={type:'pattern',pattern:'solid',fgColor:{argb:'FF0F766E'}},subFill={type:'pattern',pattern:'solid',fgColor:{argb:'FFDBEAFE'}},white={argb:'FFFFFFFF'},red={argb:'FFDC2626'},green={argb:'FF059669'};
  const styleSheet=(sheet)=>{sheet.eachRow((row,rowIndex)=>{row.height=rowIndex===1?28:22;row.eachCell({includeEmpty:true},cell=>{cell.font={name:'Calibri',size:11,bold:rowIndex===1,color:rowIndex===1?white:{argb:'FF111827'}};cell.alignment={horizontal:'center',vertical:'middle',wrapText:true};cell.border={top:{style:'thin',color:{argb:'FFD1D5DB'}},bottom:{style:'thin',color:{argb:'FFD1D5DB'}},left:{style:'thin',color:{argb:'FFD1D5DB'}},right:{style:'thin',color:{argb:'FFD1D5DB'}}};if(rowIndex===1)cell.fill=headerFill;});});};
  const uniqueIssueOrders=(list)=>new Set(list.map(item=>String(item.order?.id||item.ticket||item.event_id||''))).size;
  const overview=workbook.addWorksheet('Overview');
  overview.columns=[{width:28},{width:22},{width:22},{width:22},{width:22},{width:42}];
  overview.addRow(['Secretary Audit Management Report',`${secretaryAuditState.from} → ${secretaryAuditState.to}`,'','','','']);overview.mergeCells('A1:F1');
  overview.addRow(['Branch','Total Orders','Orders With Errors','Correct Orders','Error Rate','Management Decision']);
  branches.forEach(branch=>{const branchOrders=allOrders.filter(order=>getSecretaryAuditOrderBranch(order)===branch),branchFindings=findings.filter(item=>item.branch===branch),errorOrders=uniqueIssueOrders(branchFindings),correct=Math.max(0,branchOrders.length-errorOrders),rate=branchOrders.length?errorOrders/branchOrders.length*100:0;overview.addRow([branch,branchOrders.length,errorOrders,correct,rate/100,secretaryAuditDecision(rate)]);overview.getCell(overview.rowCount,5).numFmt='0.0%';});
  overview.addRow([]);overview.addRow(['Employee','Branch','Total Orders','Error Orders','Error Rate','Decision']);
  const employeeKeys=[...new Set(allOrders.map(order=>`${getSecretaryAuditOrderBranch(order)}|||${String(order.employee_name||'غير محدد')}`))];
  employeeKeys.forEach(key=>{const [branch,employee]=key.split('|||'),employeeOrders=allOrders.filter(order=>getSecretaryAuditOrderBranch(order)===branch&&String(order.employee_name||'غير محدد')===employee),employeeFindings=findings.filter(item=>item.branch===branch&&String(item.actor||'غير محدد')===employee),errorOrders=uniqueIssueOrders(employeeFindings),rate=employeeOrders.length?errorOrders/employeeOrders.length*100:0;overview.addRow([employee,branch,employeeOrders.length,errorOrders,rate/100,secretaryAuditDecision(rate)]);overview.getCell(overview.rowCount,5).numFmt='0.0%';});
  overview.getRow(1).fill=headerFill;overview.getRow(1).font={name:'Calibri',size:14,bold:true,color:white};overview.getRow(2).fill=subFill;overview.getRow(2).font={name:'Calibri',size:11,bold:true};const employeeHeader=branches.length+4;overview.getRow(employeeHeader).fill=subFill;overview.getRow(employeeHeader).font={name:'Calibri',size:11,bold:true};styleSheet(overview);
  branches.forEach(branch=>{
    const sheet=workbook.addWorksheet(branch.slice(0,31));
    sheet.columns=[{width:7},{width:14},{width:16},{width:24},{width:22},{width:22},{width:15},{width:15},{width:16},{width:16},{width:25},{width:52}];
    const branchFindings=findings.filter(item=>item.branch===branch).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));
    const branchOrders=allOrders.filter(order=>getSecretaryAuditOrderBranch(order)===branch);
    sheet.addRow([`${branch} — Secretary Audit`,`${secretaryAuditState.from} → ${secretaryAuditState.to}`]);sheet.mergeCells('A1:L1');
    sheet.addRow(['Employee','Total Orders','Error Orders','Correct Orders','Error Rate','Management Decision']);
    const branchEmployees=[...new Set([...branchOrders.map(order=>String(order.employee_name||'غير محدد')),...branchFindings.map(item=>String(item.actor||'غير محدد'))])].sort((a,b)=>a.localeCompare(b,'ar'));
    branchEmployees.forEach(employee=>{const employeeOrders=branchOrders.filter(order=>String(order.employee_name||'غير محدد')===employee),employeeFindings=branchFindings.filter(item=>String(item.actor||'غير محدد')===employee),errorOrders=uniqueIssueOrders(employeeFindings),correct=Math.max(0,employeeOrders.length-errorOrders),rate=employeeOrders.length?errorOrders/employeeOrders.length*100:0;const row=sheet.addRow([employee,employeeOrders.length,errorOrders,correct,rate/100,secretaryAuditDecision(rate)]);row.getCell(5).numFmt='0.0%';});
    sheet.addRow([]);
    const detailsHeaderRow=sheet.rowCount+1;
    sheet.addRow(['#','Ticket ID','Order Number','Customer','Doctor','Employee','Severity','Status','Price','Deposit','Date & Time','Detected Issue']);
    const detailsStartRow=sheet.rowCount+1;
    branchFindings.forEach((item,index)=>{const order=item.order||{},label=item.severity==='error'?'Error':item.severity==='resolved'?'Resolved':'Warning';sheet.addRow([index+1,item.ticket||'',order.order_number||'',order.customer_name||'',order.doctor_name||'',item.actor||order.employee_name||'',label,order.status||'',Number(getEffectiveOrderPrice(order)||0),Number(order.deposit||0),formatEnglishDateTime(item.created_at||order.updated_at||order.created_at||''),item.issue||'']);});
    if(!branchFindings.length)sheet.addRow(['','','','','','','','','','','','No findings for this branch']);
    styleSheet(sheet);
    [2,detailsHeaderRow].forEach(rowIndex=>{sheet.getRow(rowIndex).fill=subFill;sheet.getRow(rowIndex).font={name:'Calibri',size:11,bold:true,color:{argb:'FF111827'}};});
    branchFindings.forEach((item,index)=>{sheet.getRow(detailsStartRow+index).getCell(7).font={name:'Calibri',size:11,bold:true,color:item.severity==='resolved'?green:red};});
  });
  const buffer=await workbook.xlsx.writeBuffer(),url=URL.createObjectURL(new Blob([buffer],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'})),link=document.createElement('a');link.href=url;link.download=`Secretary-Audit-Management-${secretaryAuditState.from}_${secretaryAuditState.to}.xlsx`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1200);
  try{logActivity('data_exported',`تم تصدير تقرير Secretary Audit الإداري | ${secretaryAuditState.from} → ${secretaryAuditState.to}`);}catch(_){ }
}
function openSecretaryAuditOrder(id){
  ...
  openReportOrderTab('secretary_audit',order);
}
...
function getAuditChatBranch(order){
  // A selected external carrier wins over a stale branch assignment after transfer.
  const shipping=String(order?.shipping_company||order?.shipping_system||'').trim();
  const key=normalizeProductReportBranch(shipping||order?.branch||order?.branch_name||'');
  const branch={'nasr-city':'مدينة نصر',alexandria:'اسكندرية',tanta:'طنطا',mansoura:'المنصورة'}[key];
  return typeof branch==='string'&&PENDING_BRANCH_NAMES.includes(branch)?branch:'';
}

function isBranchAuditChatMessage(row){
  if(!Object.values(AUDIT_BOTS).some(bot=>bot.username===String(row?.sender_username||'')))return true;
  const match=String(row?.message||'').match(/(?:^|\n)الفرع:[ \t]*([^\r\n]+)/);
  return Boolean(match&&getAuditChatBranch({branch:match[1]}));
}
...
function editShippingCost() {
  alert('إجمالي مصروفات الشحنات يُحسب تلقائيًا من الأوردرات ولا يمكن تعديله يدويًا.');
}
...
function openKhaznaOrderTab(orderId){
  const order=khaznaOrders.find(item=>String(item.id)===String(orderId));
  if(order)openReportOrderTab('khazna',order);
}
...
let _collectLockedSalesValue = 0;
...
function validateCollectionDraft(order, values = {}) {
  ...
  if (!courtesyPrepaidOrder && hasUpfrontProof && deposit <= 0) errors.push('إثبات السكرتارية موجود لكن مبلغ المدفوع صفر');
...
async function openCollectModal(orderId, customerName, price, deposit, src) {
  ...
  const salesValue = _collectIsFullyPrepaid ? 0 : last
  ...
  _collectLockedSalesValue = Number(salesValue || 0);
  const collectSalesInput = document.getElementById('collectSalesInput');
  if (collectSalesInput) {
    collectSalesInput.readOnly = !isAdmin();
    collectSalesInput.setAttribute('aria-readonly', isAdmin() ? 'false' : 'true');
    collectSalesInput.style.cursor = isAdmin() ? 'text' : 'not-allowed';
...
let customerProfileFocusOrderId = '';
let customerProfileEditSaving = false;
...
function renderCustomerProfile() {
  ...
  const addNote = '<button class="customer-profile-soft-btn" type="button" onclick="openCustomerProfileOrderNote()"><span>📝 إضافة ملاحظة</span></button>';
  ...
  content.innerHTML = `
    ...
    <section id="customerProfileEditPanel" class="customer-profile-edit hidden">
      ...
      <div class="wide customer-profile-edit-actions"><button id="customerProfileEditSaveBtn" type="button" onclick="saveCustomerProfileEdit()">حفظ بيانات العميل</button><button type="button" class="cancel" onclick="toggleCustomerProfileEdit(false)">إلغاء</button></div>
    </section>
...
async function saveCustomerProfileEdit() {
  if (!isAdmin() || !customerProfileCustomer || customerProfileEditSaving) return;
  const payload = {
  ...
  const original={...customerProfileCustomer};
  const saveButton=document.getElementById('customerProfileEditSaveBtn');
  customerProfileEditSaving=true;
  if(saveButton){saveButton.disabled=true;saveButton.textContent='جاري الحفظ...';}
  try{
    const customerResult=await supabaseClient.from('customers').update(payload).eq('id',original.id).select().single();
    if(customerResult.error)throw customerResult.error;
    const orderPayload={customer_name:payload.customer_name,phone:payload.phone,phone2:payload.phone2,area:payload.area,doctor_name:payload.doctor_name};
    const ordersResult=await supabaseClient.from('orders').update(orderPayload).eq('customer_id',original.id).select('*');
    if(ordersResult.error){
      const rollback={customer_name:original.customer_name,phone:original.phone,phone2:original.phone2||null,area:original.area||null,doctor_name:original.doctor_name||null,notes:original.notes||null,last_branch:original.last_branch||null};
      await supabaseClient.from('customers').update(rollback).eq('id',original.id);
      throw ordersResult.error;
    }
    const finalCustomer=await supabaseClient.from('customers').update({...payload,last_branch:original.last_branch||null}).eq('id',original.id).select().single();
    customerProfileCustomer=finalCustomer.error?customerResult.data:finalCustomer.data;
    const updatedOrders=ordersResult.data||[];
    customerProfileOrders=updatedOrders.filter(canViewCustomerOrder).sort((a,b)=>new Date(b.created_at||0)-new Date(a.created_at||0));
    updatedOrders.forEach(order=>applyLiveOrderPayload({eventType:'UPDATE',new:order,old:{id:order.id}}));
    await logActivity('customer_updated','تم تعديل ملف العميل والأوردرات المرتبطة',`العميل: ${payload.customer_name} | الموبايل: ${payload.phone} | الأوردرات: ${updatedOrders.length}`,{customer_name:payload.customer_name,branch_name:customerProfileCustomer.last_branch||null});
    renderCustomerProfile();
    alert(`✅ تم تحديث بيانات العميل وظهورها في ${updatedOrders.length} أوردر مرتبط.`);
  }catch(error){
    alert('تعذر تعديل بيانات العميل والأوردرات المرتبطة: '+(error?.message||error));
  }finally{
    customerProfileEditSaving=false;
    const button=document.getElementById('customerProfileEditSaveBtn');
    if(button){button.disabled=false;button.textContent='حفظ بيانات العميل';}
  }
}

function openCustomerOrderFromProfile(orderId) {
  const order = customerProfileOrders.find(item => String(item.id) === String(orderId));
  if (!order) return;
  openReportOrderTab('customer_profile',order);
}
...
function applyChatConversationMetadata(rows){
  ...
  (rows||[]).filter(isBranchAuditChatMessage).forEach(row=>{
...
async function loadChatConversationMetadata(){
  ...
  const unreadRows=(received.data||[]).filter(row=>row.is_read===false&&isBranchAuditChatMessage(row));
  const counts={};
...
async function refreshChatUnreadCount(){
  ...
  const visibleRows=(data || []).filter(isBranchAuditChatMessage);
  updateChatUnreadBadge(visibleRows.length);
  const counts = {};
...
function startChatRealtime(){
  ...
  chatRealtimeChannel = supabaseClient.channel(`okb-chat-${me}-${Date.now()}`)
    .on('postgres_changes',{event:'INSERT',schema:'public',table:'chat_messages'},async payload=>{
      ...
      if(!isBranchAuditChatMessage(row))return;
...
async function loadChatMessages(){
  ...
  chatMessages=[...(a.data||[]),...(b.data||[])].filter(isBranchAuditChatMessage).sort((x,y)=>new Date(x.created_at)-new Date(y.created_at));
...
<body>
 ...
 <div style="font-size:12px;margin-bottom:4px;">رقم الأوردر: <strong>${order.order_number || '—'}</strong></div>
  ...
  <div style="display:flex;direction:rtl;justify-content:flex-start;align-items:center;gap:8px;text-align:right;font-family:Arial,sans-serif;font-size:11px;color:#555;margin-top:6px;">
    <strong style="color:#000;direction:ltr;">E - ${editCount}</strong>
...
// 👇 دي النسخة المحدثة
// ===== خزنة الفروع المجمعة =====
let branchesTreasuryOrders = [];
let branchesTreasurySelectedIds = new Set();

function getBranchesTreasuryFilters() {
  return {
...
function getBranchesTreasuryVisibleOrders(ignoreStatus = false) {
  const filters = getBranchesTreasuryFilters();
  return branchesTreasuryOrders.filter(order => {
    ...
    if (filters.search) {
      ...
      if (!searchable.includes(filters.search) && !matchesOrderSearch(order, filters.search)) return false;
    }
    return true;
  });
}

async function showBranchesTreasuryPage() {
  if (!hasRoleFeature('branches_treasury')) { alert('خزنة الفروع غير مضافة لصلاحيات حسابك'); return; }
  closeOKBStoresMenu();
  hideAllPages();
  document.getElementById('branchesTreasuryPage')?.classList.remove('hidden');
  setActiveMenu('branchesTreasuryPage');
  const today = getLocalDateISO();
...
async function loadBranchesTreasury() {
  ...
  branchesTreasurySelectedIds.clear();
  renderBranchesTreasury();
}

async function refreshBranchesTreasury(button) {
  const old = button?.textContent;
  if (button) { button.disabled = true; button.textContent = 'جاري التحديث...'; }
  try { await loadBranchesTreasury(); }
  finally { if (button) { button.disabled = false; button.textContent = old || '↻ Refresh'; } }
}

function resetBranchesTreasury() {
  const today = getLocalDateISO();
...
function toggleAllBranchesTreasury(checked) { getBranchesTreasuryVisibleOrders().forEach(order => checked ? branchesTreasurySelectedIds.add(String(order.id)) : branchesTreasurySelectedIds.delete(String(order.id))); renderBranchesTreasury(); }
function syncBranchesTreasurySelection(rows=[]) {
...
function openBranchesTreasuryOrder(id) {
  const order=branchesTreasuryOrders.find(item=>String(item.id)===String(id)); if(!order)return;
...
function commissionShiftYear(dateText,delta=-1){
  const match=String(dateText||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);if(!match)return'';
  const year=Number(match[1])+delta,month=Number(match[2]),day=Number(match[3]),lastDay=new Date(Date.UTC(year,month,0)).getUTCDate();
  return `${year}-${String(month).padStart(2,'0')}-${String(Math.min(day,lastDay)).padStart(2,'0')}`;
}
function commissionAnnualPeriodLabel(from,to){
  const months=['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'];
  const label=value=>{const match=String(value||'').match(/^(\d{4})-(\d{2})-\d{2}$/);return match&&months[Number(match[2])-1]?`${match[1]} — ${months[Number(match[2])-1]}`:'—';};
  const first=label(from),last=label(to);return first===last?first:`${first} إلى ${last}`;
}
...
async function applyCommissionDateRange(){const from=document.getElementById('commissionFromDate')?.value,to=document.getElementById('commissionToDate')?.value,compareFrom=document.getElementById('commissionCompareFromDate')?.value,compareTo=document.getElementById('commissionCompareToDate')?.value;if(!from||!to){alert('اختار الفترة الأساسية');return;}if(from>to){alert('تاريخ البداية يجب أن يكون قبل تاريخ النهاية');return;}if(commissionActiveView!=='annual'&&(!compareFrom||!compareTo)){alert('اختار فترة المقارنة');return;}if(compareFrom&&compareTo&&compareFrom>compareTo){alert('تاريخ بداية المقارنة يجب أن يكون قبل تاريخ نهايتها');return;}commissionAnnualData=null;await loadCommissionDashboard();if(commissionActiveView==='annual')await loadCommissionAnnualComparison();}
async function resetCommissionDateRange(){
  const auto=document.getElementById('commissionAutoCompare');if(auto)auto.checked=true;setCommissionCurrentMonthRange();setupCommissionBranchFilter();commissionAnnualData=null;commissionActiveView='total';document.querySelectorAll('[data-commission-view]').forEach(item=>item.classList.toggle('active',item.dataset.commissionView==='total'));await loadCommissionDashboard();
}
...
async function refreshCommissionDashboard(button){const old=button?.textContent||'Refresh';if(button){button.disabled=true;button.textContent='جاري التحديث...';}try{commissionAnnualData=null;await loadCommissionDashboard(true);if(commissionActiveView==='annual')await loadCommissionAnnualComparison(true);}finally{if(button){button.disabled=false;button.textContent=old;}}}
...
async function loadCommissionAnnualComparison(force=false){
  if(!commissionData)return;
  const range=commissionData.range,previousFrom=commissionShiftYear(range.from,-1),previousTo=commissionShiftYear(range.to,-1),annualRange=commissionDateRange(range.from,range.to,previousFrom,previousTo);if(!annualRange)return;
  const notice=document.getElementById('commissionNotice');if(notice){notice.textContent='جاري تحميل المقارنة السنوية...';notice.classList.remove('hidden');}
  try{
    const branches=commissionAccessibleBranches(),scope=(isAdmin()||canApproveCommission())?'all':branches.join('|'),key=`annual:${annualRange.from}:${annualRange.to}:${annualRange.previousFrom}:${annualRange.previousTo}:${scope}`;let data=!force?commissionRangeCache.get(key):null;
    if(!data){const [current,previous]=await Promise.all([fetchCommissionOrdersForRange(annualRange.from,annualRange.to,branches),fetchCommissionOrdersForRange(annualRange.previousFrom,annualRange.previousTo,branches)]);data=commissionBuildData(current,previous,annualRange,branches);commissionRangeCache.set(key,data);if(commissionRangeCache.size>4)commissionRangeCache.delete(commissionRangeCache.keys().next().value);}
    commissionAnnualData=data;if(commissionActiveView==='annual')renderCommissionDashboard();if(notice)notice.classList.add('hidden');
  }catch(error){if(notice){notice.textContent=`تعذر تحميل المقارنة السنوية: ${error?.message||error}`;notice.classList.remove('hidden');}}
}
function setCommissionView(view,button){commissionActiveView=['total','dashboard','comparison','annual','rtd','development'].includes(view)?view:'total';document.querySelectorAll('[data-commission-view]').forEach(item=>item.classList.toggle('active',item.dataset.commissionView===commissionActiveView));if(button)button.classList.add('active');renderCommissionDashboard();if(commissionActiveView==='annual'&&!commissionAnnualData)loadCommissionAnnualComparison();}
...
function renderCommissionDashboard(){
  ...
  if(commissionActiveView==='annual'){
    if(!commissionAnnualData){if(title)title.textContent='المقارنة السنوية — جاري التحميل';if(period)period.textContent='تتم المقارنة تلقائيًا مع نفس الفترة من السنة السابقة';if(summary)summary.innerHTML='';if(head)head.innerHTML='';if(body)body.innerHTML='<tr><td>جاري تحميل بيانات المقارنة السنوية...</td></tr>';return;}
    const annual=commissionAnnualData,annualRows=branch==='all'?annual.all:annual.rows.filter(row=>row.branch===branch),annualKpiRows=branch==='all'?annual.all:annualRows,annualTotal=key=>annualKpiRows.reduce((sum,row)=>sum+Number(row[key]||0),0);
    if(period)period.textContent=`الفترة الحالية: ${annual.range.from} → ${annual.range.to} • نفس الفترة من السنة السابقة: ${annual.range.previousFrom} → ${annual.range.previousTo}`;
    if(title)title.textContent='المقارنة السنوية — نفس الفترة من السنة السابقة';if(head)head.innerHTML=`<th>الفرع / المسؤول</th><th>${escapeHTML(commissionAnnualPeriodLabel(annual.range.previousFrom,annual.range.previousTo))}</th><th>${escapeHTML(commissionAnnualPeriodLabel(annual.range.from,annual.range.to))}</th><th>التطور السنوي</th><th>البونص الحالي</th><th>الاتجاه</th>`;
    if(summary)summary.innerHTML=commissionMiniCard('متوسط السنة السابقة',commissionFmtPct(annual.manager.previousDelivery))+commissionMiniCard('متوسط الفترة الحالية',commissionFmtPct(annual.manager.delivery))+commissionMiniCard(isAdmin()?'تطور مدير الفروع':'تطور نطاق حسابك',`${annual.manager.development>=0?'+':''}${commissionFmtPct(annual.manager.development)}`,annual.manager.development>=0?'good':'bad')+commissionMiniCard('البونص الأساسي الحالي',commissionFmtMoney(annualTotal('base')));
    if(body)body.innerHTML=annualRows.map(row=>`<tr class="${rowClass(row)}"><td>${escapeHTML(row.branch)}</td><td>${commissionFmtPct(row.previousDelivery)}</td><td>${commissionFmtPct(row.delivery)}</td><td class="${row.development>=0?'good':'bad'}">${row.development>=0?'+':''}${commissionFmtPct(row.development)}</td><td class="gold">${commissionFmtMoney(row.base)}</td><td class="status-text ${row.development>0?'good':row.development<0?'bad':'neutral'}">${row.development>0?'تحسن':row.development<0?'تراجع':'ثابت'}</td></tr>`).join('')||'<tr><td colspan="6">لا توجد بيانات</td></tr>';return;
  }
...
async function exportCommissionWorkbook(){
  ...
  const styleSheet=ws=>{ws.views=[{rightToLeft:true,state:'normal'}];ws.properties.defaultRowHeight=22;ws.eachRow((row,index)=>{row.alignment={horizontal:'center',vertical:'middle'};if(index<=4){row.font={bold:true,color:{argb:index===1?gold:white}};row.fill={type:'pattern',pattern:'solid',fgColor:{argb:index===1?black:dark}};}});};
